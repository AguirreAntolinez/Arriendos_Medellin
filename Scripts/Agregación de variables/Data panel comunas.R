...
Personas_comuna<-data_consolidada %>%
  mutate(
    base_personas=1,
    vivia_en_otro_pais = vivia_en_otro_pais * factorExpPersonas,
    vivia_en_otro_municipio = vivia_en_otro_municipio * factorExpPersonas,
    vivia_en_otro_barrio = vivia_en_otro_barrio * factorExpPersonas,
    vive_arriendo = vive_arriendo * factorExpPersonas,
    cambio_barrio = case_when(
        anios_en_barrio>=1~0,
        .default = 1),
    cambio_barrio = cambio_barrio * factorExpPersonas) %>%
  ...
  summarise(
    ...
    anios_en_barrio=weighted.mean(x = anios_en_barrio,w = factorExpPersonas,na.rm=TRUE),
...
Personas_comuna <- Personas_comuna %>%
  group_by(codigoComuna) %>%  # Agrupar por barrio/comuna
  arrange(codigoComuna, medicion) %>%  # Ordenar por año
  mutate(PoblacionAnterior = lag(Poblacion),
         ...
         Poblacion2024= max(if_else(medicion == 2024, Poblacion, NA_real_), na.rm = TRUE))  # Crear columna con valor anterior


Personas_comuna<- Personas_comuna %>% mutate(
...
Hogares_barrio<-data_consolidada   %>%
  select(codigoComuna,medicion,skHogar,valor_arriendo,factorExpHogares #,FEP_barrio
  ) %>% 
  distinct() %>% 
  group_by(codigoComuna,medicion) %>% 
...
viviendas<-viviendas %>% 
  ...
  mutate(medicion=as.character(sub("X","",medicion)),
         ...
         codigoComuna=as.character(Comuna)) %>% 
  select(codigoComuna,medicion,viviendas)%>% 
  group_by(codigoComuna,medicion) %>% 
